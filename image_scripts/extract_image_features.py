...
def map_centroids_long_lat(feat_dict):
...
def get_noaa_centroids(df, scan_year):
    ...
    OUTPUT: active region centroids x and y components and noaa active region number 
    '''
    #nova_x = []
    #nova_y = []
    y_i = None
    ...
    lat_long_values = df[scan_year == df.date].latLong.values
    clon_values = df[scan_year == df.date].clon.values 
    active_region_numbers = df[scan_year == df.date].activeRegionNum.values

    for lat, clon, ar_num in zip(lat_long_values ,clon_values, active_region_numbers):
        ...
        nova_cents.append( ([int(clon), y_i], ar_num) )
        #nova_y.append(y_i)
...
def get_neighbor_distances(ar_cents, noaa_cent_labels):
    '''
    calculate the distance between every possible pair of points: mydata points and nova data points 
    '''
    ar_data = defaultdict(list)
    for i, ar_cent in enumerate(ar_cents):
        noaaDict = dict()
        for j, noaa_cent_label in enumerate(noaa_cent_labels):
            
            noaa_cent = noaa_cent_label[0]
            noaa_ar_label = noaa_cent_label[1]
            
            dist = euclidean(ar_cent, noaa_cent)
            # if distance is less than 5 degrees, 
            # append distance and ar_cent to dict
            
            if dist <= 5:
                ar_data[i+1].append(dict({"noaa_ar_num":j, "dist":dist, "noaa_ar_label": noaa_ar_label}))
                
    return ar_data


# PRE-DEBUGGING VERSION
...
def get_shortest_distance_pair(ar_cents, nova_cent_labels):
    ar_data = dict()
    ar_dist_labels = get_neighbor_distances(ar_cents, nova_cent_labels)
    # loops through active region dicts
    for mydata_index, data_list in ar_dist_labels.iteritems():
        ...
        if len(data_list) > 1:
            ...
            # loops through key:val pairs in each data dict
            for i, data_dict in enumerate(data_list):  
                noaa = data_dict["noaa_ar_num"]
                dist = data_dict["dist"]
                label = data_dict["noaa_ar_label"]
            ...
            ar_data[mydata_index] = [dict({"noaa_ar_num":noaa_i, "dist":dist_i, "noaa_ar_label": label_i})]

           
        else:
            ar_data[mydata_index] = [data_list[0]]
    return ar_data
...
def get__noaa_ar_assignments_(mydata):
    ...
    noaa_ar = []
    for data_dict in mydata.itervalues():
        noaa_ar.append(data_dict[0]["noaa_ar_num"])
...
def get_mydata_index_for_poping_(repeating_noaa, mydata):
    ...
    for mydata_ind, data_list in mydata.iteritems():
        # check if noaa active region number  
        # is in list of repeating noaa numbers 
        data_dicts = data_list[0]
        
        if data_dicts["noaa_ar_num"] in repeating_noaa:
            
            if loop_counter == 0:
                dist_i = data_dicts["dist"]
                mydata_i = mydata_ind
            else:
                if dist_i < data_dicts["dist"]:
                    dist_i = data_dicts["dist"]
                    mydata_i = mydata_ind  
            loop_counter += 1
    return mydata_i
...
def get_min_dist_for_repeated_noaa_assignment_(repearing_noaa, data ):
    '''
    Input:  repearing_noaa, int
            data, dict
    Output: image_noaa_dist, dict
    '''
    one_to_one = dict()
    dist = None
    noaa_label = None
    ...
    for image_ind, data_list in data.iteritems():
        data_dict = data_list[0]
        
        noaa_index = data_dict["noaa_ar_num"]

        if noaa_index == repearing_noaa:
            if match_counter == 0:
                dist = data_dict["dist"]
                noaa_label =  data_dict["noaa_ar_label"]
                ar_num =  data_dict["noaa_ar_num"]
                image_id_temp = image_ind
                match_counter += 1

            else:
                if dist > data_dict["dist"]:
                    dist = data_dict["dist"]
                    noaa_label =  data_dict["noaa_ar_label"]
                    ar_num =  data_dict["noaa_ar_num"]
                    image_id_temp = image_ind
    ...
    one_to_one[image_id_temp] =\
    dict({"noaa_ar_num":ar_num, "dist":dist, "noaa_ar_label": noaa_label})
...
def check_repeating_noaa_assignments(data):
    ...
    # if True, multiple noaa ar numbers have repeating assignments to mydata ar 
	    ...
	    try:
                ...
                data_results[one_to_one.keys()] = one_to_one.values()
            except TypeError:
                pass
