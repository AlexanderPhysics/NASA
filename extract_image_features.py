...
def get_min_dist_for_repeated_noaa_assignment(repearing_noaa, data ):
    noaa_dist = dict()
    image_noaa_dist = dict()
    dist_i = None
    match_counter = 0 
    for imageInd, noaaInd_dist in data.iteritems():
        #print  imageInd, noaaInd_dist[0]
...
def get_one_to_one_assignments(data_dict, repeating_noaa_ar):
...
def check_repeating_noaa_assignments(data):
	...
	# if True, multiple noaa ar numbers have repeating assignments to mydata ar 
		...
		data_results = get_one_to_one_assignments(data, repeating_noaa_ar)
		for repeating_noaa in repeating_noaa_ar:
		    one_to_one = get_min_dist_for_repeated_noaa_assignment(repeating_noaa, data)
