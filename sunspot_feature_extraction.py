...
def identify_active_regions(labels): 
    ...
    '''
    pseudo_act_reg= 100 # needs to be confirmed with David
    count_ar = defaultdict(list)
    for row in labels:
        for elem in row:
            count_ar[elem].append(elem)
            
    # Need a pixel threshold for filtering pseudo-active regions\
    # let's try 100 pixels
    # index 0 is for the background 
    
    active_labels = []
    for key, val in count_ar.iteritems():
        # exclude pseudo-active regions and exclude zeroed-out background
        if Counter(val)[key] >= pseudo_act_reg and np.sum(val) > 1 :
...
def extract_features(labels,image, hdu, file_name):
